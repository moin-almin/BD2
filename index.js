...
function sortHotelByPrice(hotels, pricing) {
  if (pricing === 'low-to-high') {
    return  hotels.sort(sortHotelsByPriceLowToHigh);
  } else if (pricing === 'high-to-low') {
    return  hotels.sort(sortHotelsByPriceHighToLow);
  }
};
...
function sortHotelsByRating(hotels, rating) {
  if (rating === 'low-to-high') {
    return hotels.sort(sortHotelsByRatingLowToHigh);
  } else if (rating === 'high-to-low') {
    return hotels.sort(sortHotelsByRatingHighToLow);
  }
};
...
function sortHotelsByReviews(hotels, rating) {
  if (rating === 'least-to-most') {
    return hotels.sort(sortHotelsByReviewsLeastToMost);
  } else if (rating === 'most-to-least') {
    return hotels.sort(sortHotelsByReviewsMostToLeast);
  }
};
...
app.get('/hotels/sort/pricing', (req, res) => {
  ...
  let result = sortHotelByPrice(hotelsCopy, pricing);
...
app.get('/hotels/filter/amenity', (req, res) => {
  ...
  let result = hotels.filter(hotelObj => filterHotelsByAmenity(hotelObj, amenity));
  res.json({ hotels: result });
});

app.get('/hotels/filter/country', (req, res) => {
  let country = req.query.country;
  let result = hotels.filter(hotelObj => filterHotelsByCountry(hotelObj, country));
  res.json({ hotels: result });
})

app.get('/hotels/filter/category', (req, res) => {
  let category = req.query.category;
  let result = hotels.filter(hotelObj => filterHotelsByCategory(hotelObj, category));
  res.json({ hotels: result });
})
